...
func query(ctx context.Context, client influxdb2.Client, influxOrg string, influxBucket string, measurement string) error {
	...
	|> filter(fn: (r) => r["_field"] == "temperature" or r["_field"] == "rain")
	|> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")`, influxBucket, measurement)
	...
	for result.Next() {
		...
		// Access data
		temp := result.Record().ValueByKey("temperature")
		rain := result.Record().ValueByKey("rain")
		country := result.Record().ValueByKey("country")
		location := result.Record().ValueByKey("location")
		tbl := result.Record().Table()

		fmt.Printf("table:%v timestamp:%v country:%v location:%v temp:%v rain:%v\n", tbl, result.Record().Time().Unix(), country, location, temp, rain)
	}
	return nil
}
